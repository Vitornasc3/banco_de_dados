...
SELECT * FROM tb_produtos WHERE nome LIKE "%ra%"; -- qualquer lugar

SELECT * FROM tb_produtos WHERE nome LIKE "ra%"; -- termina com

SELECT * FROM tb_produtos WHERE nome LIKE "%ra"; -- começa com
...
SELECT tb_categorias.descricao, CONCAT('R$ ', FORMAT(avg(preco), 2, 'pt-BR')) AS Preço_Medio  -- Seleciona os dados
FROM tb_produtos INNER JOIN tb_categorias                                                     -- Seleciona as tabelas
ON categoria_id = tb_categorias.id                                                            -- Seleciona a ligação de uma tabela com a outra
GROUP BY tb_categorias.descricao;                                                             -- Seleciona a forma de agrupamento (Sempre necessário quando é feita operação matemática)